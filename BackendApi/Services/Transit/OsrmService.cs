...
public sealed class OsrmService : IOsrmService
{
    ...
    public async Task<(bool Success, LineString? RouteGeometry, string? Error)>
        GetRouteAsync(IEnumerable<Coordinate> orderedCoordinates)
    {
        var coords = orderedCoordinates.ToList();

        if (coords.Count < 2)
            return (false, null, "Rota oluşturmak için en az 2 durak gereklidir.");
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                return (false, null, "OSRM servisinden rota alınamadı.");
            }
            ...
            if (code != "Ok")
            {
                ...
                return (false, null, "Belirtilen duraklar arasında rota bulunamadı.");
            }

            var coordinatesJson = json.RootElement
            ...
            return (true, lineString, null);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "OSRM routing request failed");
            return (false, null, "Sunucu hatası.");
        }
    }
}